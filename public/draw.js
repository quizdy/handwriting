...
$(document).on('touchend mouseup', '.whiteboard>canvas.draw', (e) => {
    ...
    socket.emit('sender_draw_stop', _line, canvas.height, canvas.width)
})
...
function drawing(canvas, line) {
    ...
    socket.emit('sender_draw', line, canvas.height, canvas.width)
}
...
socket.on('recieve_draw_start', (res) => {
    console.info('recieve_draw_start')
    _rev = res
})
...
socket.on('recieve_draw_stop', (res, height, width) => {
    console.info('recieve_draw_stop')
    _rev.x1 = res.x1
    _rev.y1 = res.y1
    const canvas = $('.reciever canvas.draw').get(0)
    revDrawing(canvas, _rev, height, width)
...
function revDrawing(canvas, line, height, width) {
    ...
    const x0 = $(canvas).width() / width * line.x0
    const x1 = $(canvas).width() / width * line.x1
    const y0 = $(canvas).height() / height * line.y0
    const y1 = $(canvas).height() / height * line.y1
